...
disposeLeafCore args =
  case inferIssueId (dlaName args) of
    Just issueId -> do
      ...
      result <- chainlinkIssueCloseCore (ChainlinkIssueCloseArgs issueId summary (dlaForce args) False)
      case result of
        Left err -> pure $ Left err
        Right _ -> pure $ Right $ object ["success" .= True, "agent" .= dlaName args, "issue_id" .= issueId, "force" .= dlaForce args]
    Nothing
      | dlaForce args -> cleanupOrphanCore (CleanupOrphanArgs (dlaName args) Nothing False False False False Nothing)
