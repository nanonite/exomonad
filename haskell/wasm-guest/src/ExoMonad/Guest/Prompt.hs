    ...
    [ Headed "Completion Protocol (Worker)" $
        T.intercalate
          ...
          [ "You are an **ephemeral worker** — you run in the parent's directory on the parent's branch. You do NOT have your own worktree or branch.",
            ...
            "   - If you are externally blocked and need a human decision or dependency repair, call `notify_parent` with status `blocked` and provide the typed blocker fields (cause, scope, retry policy, recovery action, and evidence). Use `failure` only when the assignment itself failed.",
            "",
            "**DO NOT:**",
