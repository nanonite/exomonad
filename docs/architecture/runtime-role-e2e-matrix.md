...
| Codex-Fugu role launch | Codex-Fugu root, worker, reviewer, companion | Local runtime | #531; deterministic fake-CLI checks | Role selection, `fugu`/`fugu-ultra` model validation, high-only effort, and pre-side-effect PATH validation are covered without provider credentials. |
