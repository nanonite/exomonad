...
| Codex-Fugu role launch | Codex-Fugu root, worker, reviewer, companion | Local runtime | `just check-e2e-codex-fugu`; `just e2e-codex-fugu` | Role selection, `fugu`/`fugu-ultra` model validation, high-only effort, and pre-side-effect PATH validation are covered without provider credentials. |
