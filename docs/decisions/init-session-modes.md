...
A gate answer that changes the gate's status advances the run's convergence
epoch (`state_version`), so a live controller's in-memory livelock guard
re-evaluates the released action instead of re-detecting it as a repeat and
re-parking it before the effect runs. Re-answering a gate with its current
status is idempotent and leaves the epoch unchanged, so a duplicate approval
cannot grant the same action a second retry without a new pending occurrence.
