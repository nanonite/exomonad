...
5. Inbox writer (`claude-teams-bridge/src/inbox.rs`) expects directories to exist; returns error if not (delivery helper falls back to tmux)
...
Codex calls notify_parent(status="success", message="...")
  ...
  → If Teams fails, falls back to tmux injection into parent pane
```
...
|-----------|--------|------|
| Delivery helper (Teams + tmux) | Built | `services/delivery.rs` |
...
| teams_mailbox (array append) | Built | `claude-teams-bridge/src/inbox.rs` |
| Push notification pipeline | Built | tmux buffer pattern |
