...
| `[STUCK: pr, rounds]` | `.exo/roles/devswarm/context/root.md:69-72`; `rust/exomonad-core/src/services/worktree_event_watcher.rs:642-645,3148-3178` | `pr.review` plus `agent.notify_parent` handoff | legacy input | The watcher still accepts legacy stuck wakeups, but no longer derives or enforces the ceiling; `tl_loop` emits `tl.review_rounds_exhausted`. |
...
| `stuck` | `haskell/wasm-guest/src/ExoMonad/Guest/Events.hs:67-70,113`; `rust/exomonad-core/src/services/worktree_event_watcher.rs:3148-3178` | `pr.review` | legacy input | The parser preserves this compatibility variant; round-ceiling enforcement and telemetry are controller-owned. |
...
| `tl.review_rounds_exhausted` | `tl_loop/loop/driver.py:_park_review_rounds_exhausted` | covered | The controller records the durable per-slice round count and configured ceiling when it parks and opens the human gate. |
...
| `tl.handoff_reconciled` | `tl_loop/loop/driver.py` | covered | Restart reconciliation records a deduplicated host-publication backfill or the exact missing provenance condition. |
...
| `agent.recovery.started`, `agent.recovery.outcome` | `tl_loop/loop/driver.py:_record_task_blocked_recovery`; `tl_loop/loop/heartbeat.py:_reconcile_recovery`; `rust/exomonad-core/src/handlers/tl.rs` | covered | Recovery identity is correlated by slice/invocation/generation; aggregate dimensions retain bounded attempts, rounds, depth, sibling impact, authorization, policy decision, and separate wait-time buckets. |
