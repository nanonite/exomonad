...
- Add exhaustive recursive orchestration model and migration coverage (#1056)
...
- Separate slice integration from durable post-merge recovery (#1051)
- Refactor root and non-root TL Mealy FSMs (#1050)
...
- exomonad watch dashboard freezes on slow/unbounded Forgejo calls (#1033)
...
- Adopt publication ownership across session restart (--recreate orphan) (#986)
- Add startup and heartbeat merge reconciliation (#983)
...
- Separate execution, recovery, and run timeout semantics (#959)
...
- Add typed blocked handoffs to the dev MCP contract (#945)
- Configurable per-task time budgets with auditable kill and typed failure events (#927)
