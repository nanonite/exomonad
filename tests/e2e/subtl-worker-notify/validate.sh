...
active_root_pane() {
    tmux list-panes -t "$SESSION:$ROOT_WINDOW" -F '#{pane_index}:#{pane_active}' 2>/dev/null         | awk -F: '$2 == 1 { print $1; exit }'
}

main() {
    wait_for "root Codex window exists" "tmux list-windows -t '$SESSION' -F '#{window_name}' 2>/dev/null | grep -Fxq '$ROOT_WINDOW'"
    wait_for "worker routing metadata exists" "[[ -f '$REPO_DIR/.exo/agents/$WORKER_AGENT/routing.json' ]]"
    wait_for "root tmux window exists" "tmux list-windows -t '$SESSION' -F '#{window_name}' 2>/dev/null | grep -Fxq '$ROOT_WINDOW'"
    ...
    wait_for "notification reached root pane zero" "tmux capture-pane -p -t '$SESSION:$ROOT_WINDOW.0' -S -2000 2>/dev/null | grep -Fq '$MESSAGE_MARKER'"

    pane_count="$(tmux list-panes -t "$SESSION:$ROOT_WINDOW" 2>/dev/null | wc -l | tr -d ' ')"
    if [[ "$pane_count" -lt 2 ]]; then
        record_failure "expected root window to contain a worker pane"
    fi

    active_pane="$(active_root_pane || true)"
    if [[ -n "$active_pane" && "$active_pane" != "0" ]]; then
        log "OK: worker pane was active while pane-zero delivery was observed"
    else
        record_failure "expected active root pane to be a worker pane, got '${active_pane:-unknown}'"
    fi

    {
        printf 'Worker notify E2E validation completed at %s
' "$(date -Iseconds)"
        printf 'Session: %s
' "$SESSION"
        printf 'Repo: %s
