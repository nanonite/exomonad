...
This test validates OpenCode as root TL. The OpenCode agent receives its task via `initial_prompt` in `config.toml`, delivered through the OpenCode serve/attach chain (`opencode serve` → port capture → `opencode run --attach`). OpenCode does not read `.claude/rules/` — its instructions come entirely from `initial_prompt`.
