...
WORK_DIR="$(mktemp -d /tmp/exomonad-e2e-tangled-pr-codex.XXXXXXXX)"
...
docker exec "$KNOT_CONTAINER" sh -c \
  "mkdir -p /home/git/repositories/owner/$REPO_NAME.git && \
   git init --bare /home/git/repositories/owner/$REPO_NAME.git >/dev/null 2>&1 || true && \
   chown -R git:git /home/git/repositories/owner/$REPO_NAME.git && \
   mkdir -p '/home/git/repositories/$OWNER_DID' && \
   ln -sfn '../owner/$REPO_NAME.git' '/home/git/repositories/$OWNER_DID/$REPO_NAME'"

git remote add tangled "git@local-tangled:repositories/owner/$REPO_NAME.git"
