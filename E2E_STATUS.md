...
| Codex-Fugu effort/role matrix | `just check-e2e-codex-fugu` / `just e2e-codex-fugu` | Fake-CLI init fixture covers root, worker, reviewer, and companion Fugu selection, high-only generated effort, Fugu model names, PATH preflight, and pre-side-effect medium rejection | Passed on 2026-07-16 with all role rejections and high-only config assertions green. | Green | Deterministic and credential-free; no live Fugu command sends `medium`. |
