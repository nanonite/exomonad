...
| Claude-only bounded smoke | Manual temp-repo smoke under #421 | Claude Code root TL on Haiku with explicit `initial_prompt`; verifies server startup, SessionStart registration, and TeamCreate without spawning children | Partial pass on 2026-05-27: server started on `port = 0`; root SessionStart registered; TeamCreate succeeded and registered `claude-smoke-team`; marker file was not written because TL role policy correctly denied direct `Write`. | Blocked | Full worker/dev-leaf/reviewer matrix was not run to avoid token burn. Follow-up #429 should add a role-safe harness that validates through Teams/logs or worker-owned markers, and isolates the webhook port from active sessions. |
