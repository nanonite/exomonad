...
| Tangled VM PR integration | Not available yet | Real Tangled VM / PR integration path | Not run. | Deferred | The Tangled VM PR integration path does not exist yet. Keep this separate from local E2E; local tests should use `.exo/prs.json` and must not require `gh` or external PR state. |
